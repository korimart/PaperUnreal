...
public:
	TLiveDataView<AAreaActor*> GetAreaFor(int32 TeamIndex) const
	{
		return TeamToAreaMap.FindOrAdd(TeamIndex);
	}

	TLiveDataView<TSet<AAreaActor*>> GetSpawnedAreas() const
	{
		return SpawnedAreas;
	}
	...
	AAreaActor* SpawnAreaAtRandomEmptyLocation(int32 TeamIndex)
	{
		...
		SpawnedAreas.AddSilent(Ret);
		TeamToAreaMap.FindOrAdd(TeamIndex).SetValue(Ret);
		SpawnedAreas.Notify();

		// TODO aareaactor의 팀 정보가 변경될 때마다 TeamToAreaMap의 캐시된 팀 관계도 업데이트 해야함
...
private:
	// TODO weak pointer
	mutable TLiveData<TSet<AAreaActor*>> SpawnedAreas{TSet<AAreaActor*>{}};
	mutable TMap<int32, TLiveData<AAreaActor*>> TeamToAreaMap;
};
